        ...
        <div className="flex flex-1 flex-col gap-4 p-4 pt-4 relative">
          ...
          {!error && (
            <>
              ...
              {/* Debug Panel */}
              {debugInfo && debugInfo.primaryMethod && (
                <div className="mt-8 p-4 bg-gray-900 border border-gray-700 rounded-lg">
                  ...
                  {/* Primary Method */}
                  <div className="mb-4">
                    <h4 className="text-md font-medium text-white mb-2">Primary Fetch Method:</h4>
                    <span className="px-3 py-1 bg-blue-600 text-white rounded-md text-sm">
                      {debugInfo.primaryMethod || 'unknown'}
                    </span>
                  </div>
              
                  {/* Distribution by Fetch Logic */}
                  {debugInfo.distribution && (
                    <div className="mb-4">
                      <h4 className="text-md font-medium text-white mb-2">Jobs by Fetch Method:</h4>
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                        <div className="text-center p-2 bg-gray-800 rounded">
                          <div className="text-xs text-gray-400">JobMap</div>
                          <div className="text-lg font-bold text-white">{debugInfo.distribution.jobMap || 0}</div>
                        </div>
                        <div className="text-center p-2 bg-gray-800 rounded">
                          <div className="text-xs text-gray-400">Text Search</div>
                          <div className="text-lg font-bold text-white">{debugInfo.distribution.textSearch || 0}</div>
                        </div>
                        <div className="text-center p-2 bg-gray-800 rounded">
                          <div className="text-xs text-gray-400">Skills Match</div>
                          <div className="text-lg font-bold text-white">{debugInfo.distribution.skillsMatching || 0}</div>
                        </div>
                        <div className="text-center p-2 bg-gray-800 rounded">
                          <div className="text-xs text-gray-400">Fuzzy Scoring</div>
                          <div className="text-lg font-bold text-white">{debugInfo.distribution.fuzzyScoring || 0}</div>
                        </div>
                        <div className="text-center p-2 bg-gray-800 rounded">
                          <div className="text-xs text-gray-400">Last Resort</div>
                          <div className="text-lg font-bold text-white">{debugInfo.distribution.lastResort || 0}</div>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {/* Matching Statistics */}
                  {debugInfo.matchingStats && (
                    <div className="mb-4">
                      <h4 className="text-md font-medium text-white mb-2">Jobs by Matching Type:</h4>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                        <div className="text-center p-2 bg-green-900/30 border border-green-500/30 rounded">
                          <div className="text-xs text-green-400">Exact Matches</div>
                          <div className="text-lg font-bold text-green-300">{debugInfo.matchingStats.exactMatches || 0}</div>
                        </div>
                        <div className="text-center p-2 bg-blue-900/30 border border-blue-500/30 rounded">
                          <div className="text-xs text-blue-400">Fuzzy Matches</div>
                          <div className="text-lg font-bold text-blue-300">{debugInfo.matchingStats.fuzzyMatches || 0}</div>
                        </div>
                        <div className="text-center p-2 bg-purple-900/30 border border-purple-500/30 rounded">
                          <div className="text-xs text-purple-400">Title Matches</div>
                          <div className="text-lg font-bold text-purple-300">{debugInfo.matchingStats.titleMatches || 0}</div>
                        </div>
                        <div className="text-center p-2 bg-orange-900/30 border border-orange-500/30 rounded">
                          <div className="text-xs text-orange-400">Enhanced</div>
                          <div className="text-lg font-bold text-orange-300">{debugInfo.matchingStats.enhancedMatches || 0}</div>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {debugInfo.fuzzyScoreDistribution && (
                    <div className="mb-4">
                      <h4 className="text-md font-medium text-white mb-2">Fuzzy Score Distribution:</h4>
                      <div className="grid grid-cols-3 gap-2">
                        <div className="text-center p-2 bg-red-900/30 border border-red-500/30 rounded">
                          <div className="text-xs text-red-400">High (≥0.8)</div>
                          <div className="text-lg font-bold text-red-300">{debugInfo.fuzzyScoreDistribution.high || 0}</div>
                        </div>
                        <div className="text-center p-2 bg-yellow-900/30 border border-yellow-500/30 rounded">
                          <div className="text-xs text-yellow-400">Medium (0.5-0.8)</div>
                          <div className="text-lg font-bold text-yellow-300">{debugInfo.fuzzyScoreDistribution.medium || 0}</div>
                        </div>
                        <div className="text-center p-2 bg-gray-800 border border-gray-600 rounded">
                          <div className="text-xs text-gray-400">Low (<0.5)</div>
                          <div className="text-lg font-bold text-gray-300">{debugInfo.fuzzyScoreDistribution.low || 0}</div>
                        </div>
                      </div>
                    </div>
                  )}
